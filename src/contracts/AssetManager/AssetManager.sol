...
interface IPlatformFeeReceiver {
...
contract AssetManager is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    */
    ...
    // Function to set the platform fee receiver.
    function setPlatformFeeReceiver(address _platformFeeReceiver) external onlyOwner addressIsNotZero(_platformFeeReceiver) {
 ...
 * @notice Processes the payment for a Dutch auction.
   ...
   */
    ...
    function sendProtocolFeeToTreasure(uint256 _fee) internal {
        if (_fee > 0) {
            (bool success, ) = platformFeeReceiver.call{value: _fee}("");

...
contract PaymentManager is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    */
    ...
    address payable public veBlockzAddress;

    // The percentage of income to be allocated to veBlockz contract.
    uint96 public veBlockzPercentage;
    ...
    event SetVeBlockzPercentage(uint96 _veBlockzPercentage);
    event SetVeBlockzAddress(address indexed veBlockzAddress);
    ...
    /**
    * @notice sets the percentage of income to be allocated to the veBlockz contract
    * @param _veBlockzPercentage The percentage of income to be allocated to veBlockz contract.
    */
    function setVeBlockzCommissionPercentage(uint96 _veBlockzPercentage) external onlyOwner {
        veBlockzPercentage = _veBlockzPercentage;
        emit SetVeBlockzPercentage(_veBlockzPercentage);
    }

    /**
    * @notice sets the address of the veBlockz contract
    * @param _address The address of the veBlockz contract.
    */
    function setVeBlockzAddress(address _address) external onlyOwner {
        veBlockzAddress = payable(_address);
        emit SetVeBlockzAddress(_address);
    }
    ...
    */
    function payout(
        ...
        if (commission > 0) {
            ...
            if (veBlockzAddress != address(0x0)) {
                uint256 veBlockzComissionPart = _getPortionOfBid(commission, veBlockzPercentage);
                commission -= veBlockzComissionPart;
                _safeTransferTo(veBlockzAddress, veBlockzComissionPart);
            }

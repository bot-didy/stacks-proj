...
describe("Art Staking", function () {
  before(async function () {
    ...
    this.erc721BlockzMiniCF = await ethers.getContractFactory("BlockzMini");
  ...
  beforeEach(async function () {
    ...
    await this.blockzGovernanceToken.initialMint(receivers, [
      totalToken,
      totalToken,
      totalToken,
      totalToken,
    ...
    this.erc721BlockzMini = await this.erc721BlockzMiniCF.deploy(
      this.owner.address
    );
  });

  it("deploy", async function () {
    const tempVeART = await upgrades.deployProxy(this.veARTCF, [
      this.blockzGovernanceToken.address,
    ]);
    await expect(
      tempVeART.initialize(this.blockzGovernanceToken.address)
  ...
  it("pause should set paused true and unpause should set false", async function () {
    ...
    await expect(
      this.veART.connect(this.signers[1]).pause()
    ).to.be.revertedWith("Ownable: caller is not the owner");
    await expect(
      this.veART.connect(this.signers[1]).unpause()
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });

  it("set max cap", async function () {
    await expect(
      this.veART.connect(this.signers[1]).setMaxCap(10)
    ).to.be.revertedWith("Ownable: caller is not the owner");
  ...
  it("set art reward generation rate", async function () {
    await expect(
      this.veART.connect(this.signers[1]).setARTGenerationRate(10)
    ).to.be.revertedWith("Ownable: caller is not the owner");

    await expect(this.veART.setARTGenerationRate(10)).to.emit(
      this.veART,
      "ArtGenerationRateUpdated"
    );
  });
  ...
  it("Get Boosted Generation Rate", async function () {
    expect(
      await this.veART.getBoostedGenerationRate(this.signers[1].address)
    ).to.be.equal(await this.veART.veARTgenerationRate());
  });

  it("name", async function () {
    expect(await this.veART.name()).to.be.equal("BlockzVeArt");
  });
  ...
  it("ether balance", async function () {
    expect(await this.veART.getBalance()).to.be.equal(
  ...
  it("deposit art", async function () {
    ...
    await this.veART.pause();
    await expect(this.veART.depositART(0)).to.be.revertedWith(
      "Pausable: paused"
    );
    await this.veART.unpause();

    // ensures that the call is not made from a smart contract, unless it is on the whitelist.
    await expect(this.erc721Collectible.depositART(0)).to.be.revertedWith(
      "Error: Unauthorized smart contract access"
    );
    ...
    await expect(
      this.veART.depositART("2000000000000000000000000")
    ).to.be.revertedWith(
      "Error: Insufficient balance to deposit the specified amount"
    );

    await this.blockzGovernanceToken.approve(
      this.veART.address,
    ...
    expect(this.veART.depositART("900000000000000000000000")).to.emit(
      this.veART,
      "DepositART"
    );
  });
  ...
  it("withdraw art", async function () {
    ...
    await this.veART.pause();
    await expect(this.veART.withdrawART(0)).to.be.revertedWith(
      "Pausable: paused"
    );
    await this.veART.unpause();

    await expect(this.veART.withdrawART(0)).to.be.revertedWith(
      "Error: amount to withdraw cannot be zero"
    );
    await expect(
      this.veART.withdrawART("2000000000000000000000000")
    ...
    await this.blockzGovernanceToken
      ...
      .approve(this.veART.address, "99000000000000000000000");
    await this.veART
      .connect(this.signers[2])
      .depositART("99000000000000000000000");
    ...
    await expect(this.veART.withdrawART("98000000000000000000000")).to.emit(
      this.veART,
      "WithdrawART"
    );

    await expect(
      this.veART.connect(this.signers[2]).withdrawART("99000000000000000000000")
    ).to.emit(this.veART, "WithdrawART");
  });

  it("withdraw all art", async function () {
    await this.veART.pause();
    await expect(this.veART.withdrawAllART()).to.be.revertedWith(
      "Pausable: paused"
    );
    await this.veART.unpause();
    ...
    await expect(this.veART.withdrawAllART()).to.emit(
      this.veART,
      "WithdrawART"
    );
  ...
  it("harvest veART", async function () {
    await this.veART.pause();
    await expect(
      this.veART.harvestVeART(this.owner.address)
    ).to.be.revertedWith("Pausable: paused");
    await this.veART.unpause();

    await expect(
      this.veART.harvestVeART(this.owner.address)
    ).to.be.revertedWith("Error: user has no stake");
    ...
    await expect(this.veART.harvestVeART(this.owner.address)).to.emit(
      this.veART,
      "ClaimedVeART"
    );
  });
  ...
  it("pending reward", async function () {
    ...
    expect(
      Number((await this.veART.pendingRewards(this.owner.address)).toString())
    ).to.be.gt(0);
  });
  ...
  it("pending art reward", async function () {
    expect(
      Number(
        (await this.veART.pendingARTRewards(this.owner.address)).toString()
      )
    ).to.be.eq(0);
  ...
  it("claim earnings", async function () {
    await this.veART.pause();
    await expect(
      this.veART.claimEarnings(this.owner.address)
    ).to.be.revertedWith("Pausable: paused");
    await this.veART.unpause();
    ...
    await expect(this.veART.claimEarnings(this.owner.address))
      ...
      .and.to.emit(this.veART, "ClaimARTReward");
